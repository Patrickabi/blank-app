...
def train_rf_model():
    ...
    # Separate the dataframe in 'good' (>3) or 'bad' (<3)
    df_less_than_4 =    df[df['rating'] < 3]
    df_greater_than_4 = df[df['rating'] >= 4 ]

    # Sample the dataframe with equal parts, using the inputed sample number
    sample_less_than_4 = df_less_than_4.sample(n=n, random_state=42)
    sample_greater_than_4 = df_greater_than_4.sample(n=n, random_state=42)

    # Combine the entire dataframe
    combined_sample = pd.concat([sample_less_than_4, sample_greater_than_4])
    # Reset index
    df = combined_sample.reset_index(drop=True)
    ...
    # Standardize 'Item_Weight_n'
    df['Item_Weight_n'] = scaler.fit_transform(df[['Item_Weight_n']])
    df['volume'] = scaler_.fit_transform(df[['volume']])
